...
def _conv2d_relu(vgg_layers, prev_layer, layer, layer_name):
    ...
    """
    weights = tf.constant(_weights(vgg_layers, layer, layer_name)[0])
    biases = tf.constant(_weights(vgg_layers, layer, layer_name)[1])

    conv = tf.nn.conv2d(prev_layer, weights, strides = [1, 1, 1, 1], padding = 'SAME')
