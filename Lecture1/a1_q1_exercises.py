...
fadd = lambda: tf.add(x, y)
fsub = lambda: tf.subtract(x, y)
fzero = lambda: tf.constant(0)
out = tf.case([(tf.less(x,y), fadd), (tf.greater(x,y), fsub)], default=fzero)
...
x = tf.constant([29.05088806,  27.61298943,  31.19073486,  29.35532951,
 ...
 27.88236427,  20.56035233,  30.20379066,  29.51215172,
 33.71149445,  28.59134293,  36.05556488,  28.66994858])
y = tf.constant(30, dtype=tf.float32)
...
out = tf.where(tf.less(tf.reduce_mean(sub), 0), mse, tf.reduce_sum(residual))
